...
Data(products).Scenario('Buy 1 product',  async ({ I, current }) => {
    ...
    I.see('Product Code');
    let price: String = (await I.grabTextFrom(productPage.priceLabel)).split('$')[1].split('.')[0];
    await ProductPage.selectOption(current.color, current.size);
    ProductPage.addToCart();
    ProductPage.openCart();
    I.see("My Cart");
    await CartPage.proceedToCheckout();
    let checkoutPrice: String = await CheckoutPage.placeOrder();
    I.assert.equal(price, checkoutPrice);
